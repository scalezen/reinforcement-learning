...
def price_final_date_only(
    S0, K, r, sigma, T, n_paths_train, n_paths_eval, seed, continuation_val=None
):
    """
    ...
    S_T = torch.tensor(S_T[:, -1], dtype=torch.float32).reshape(
        (n_paths_train, 1)
    )  # S_T is a torch tensor
    ...
    if not continuation_val:
        ...
        continuation = torch.full(
            (n_paths_train,), continuation_val, dtype=torch.float32
        )
    ...
    S_T_eval = torch.tensor(S_T_eval[:, -1], dtype=torch.float32).reshape(
        n_paths_eval, 1
    )  # S_T_eval is a torch tensor
    ...
    with torch.no_grad():
        ...
        continuation = torch.full(
            (n_paths_eval,), continuation_val, dtype=torch.float32
        )
        payoff_conditioned = payoff_eval.squeeze(-1) * exercise + continuation.squeeze(
            -1
        ) * (1 - exercise)
        ...
        logger.info(
            f"disagreement rate between the net and analytically = {1 - count_agreement}"
        )
...
def price_bermudan_two_dates(
    S0, K, r, sigma, T, n_paths_train, n_paths_eval, seed_train, seed_eval, n_epochs=50
):
    """
    ...
    with torch.no_grad():
        ...
        exercise_decision = (
            stopping_probabilities > 0.5
        ).float()  # 0.0 or 1.0, bool converted to float for later

        payoff_stop_at_T1 = math.exp(-r * T / 2) * torch.clamp(K - S_T1, 0.0).squeeze(
            -1
        )
        ...
        assert (
            test_condition.sum() == 0
        ), "Weird result, where network stops at T1 when payoff at T1 is 0.0"

        payoff = (
            payoff_stop_at_T1 * exercise_decision
            + (1 - exercise_decision) * payoff_stop_at_T2
        )
